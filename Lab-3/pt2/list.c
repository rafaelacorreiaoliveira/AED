...
void escreverRev(t_lista *cabecinha, FILE *fp, double num)
{
  t_lista *aux = cabecinha;

  if (aux != NULL)
  {
    escreverRev(getProxElementoLista(cabecinha), fp, num);
    escreveUmaPalavra((t_palavra *)getItemLista(aux), fp, num);
  }
  return;
}